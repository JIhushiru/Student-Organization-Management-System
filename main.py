...
main_frame = None  # Global reference
entry_username = None
entry_password = None
...
def server_program():
    ...
    def handle_connection(c):
        try:
            ...
            try: c.send("Server error.".encode())
            except: pass
        finally:
            c.close()
...
def perform_login():
    ...
    if status == "ADMIN_LOGIN_SUCCESS":
        root.after(0, lambda: switch_to_panel(open_superadmin_panel, root, username, show_login_panel)) 
    elif status == "PRESIDENT_LOGIN_SUCCESS":
        root.after(0, lambda: switch_to_panel(open_president_panel, root, False, org_name, omid, username,show_login_panel))
    elif status == "MEMBER_LOGIN_SUCCESS":
        root.after(0, lambda: switch_to_panel(show_member_fee_panel, root, omid, username, show_login_panel))
    else:
...
def show_login_panel():
    global main_frame, entry_username, entry_password
    for widget in root.winfo_children():
        widget.destroy()
    ...
    message_label = tk.Label(right_content,
        text="Manage your student organization's data with ease.\nPlease log in to continue.",
        font=("Arial", 14), bg="#020325", fg="white", justify="left", wraplength=500)
